...
BEGIN
    ...
    BEGIN TRY
        -- Load Constituent
        INSERT INTO Stg_Constituent (BBCRM_ID, FirstName, LastName, Email, DateChanged, LoadTimestamp, LoadBatchID)
        SELECT BBCRM_ID, FirstName, LastName, Email, DateChanged, GETDATE(), @BatchID
        FROM dbo.BBCRM_Constituent;
        SET @TotalInserted += @@ROWCOUNT;

        -- Load Address
        INSERT INTO Stg_Address (BBCRM_ID, AddressLine1, City, State, Zip, DateChanged, LoadTimestamp, LoadBatchID)
        SELECT BBCRM_ID, AddressLine1, City, State, Zip, DateChanged, GETDATE(), @BatchID
        FROM dbo.BBCRM_Address;
        SET @TotalInserted += @@ROWCOUNT;

        -- Load Interaction
        INSERT INTO Stg_Interaction (BBCRM_ID, InteractionDate, InteractionTypeID, Notes, DateChanged, LoadTimestamp, LoadBatchID)
        SELECT BBCRM_ID, InteractionDate, InteractionTypeID, Notes, DateChanged, GETDATE(), @BatchID
        FROM dbo.BBCRM_Interaction;
        SET @TotalInserted += @@ROWCOUNT;

        -- Load Address Type
        INSERT INTO Stg_Address_Type (AddressTypeID, Description, DateChanged, LoadTimestamp, LoadBatchID)
        SELECT AddressTypeID, Description, DateChanged, GETDATE(), @BatchID
        FROM dbo.BBCRM_Address_Type;
        SET @TotalInserted += @@ROWCOUNT;

        -- Load Interaction Type
        INSERT INTO Stg_Interaction_Type (InteractionTypeID, Description, DateChanged, LoadTimestamp, LoadBatchID)
